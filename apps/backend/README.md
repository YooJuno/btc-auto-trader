...
- 매수: 추세 게이트(MA/기울기/ADX/거래량) + 돌파 + 상위 시간대 확인이 **모두** 충족되면 시장가 매수
  - RSI/MACD/볼린저는 계산·기록만 하고 진입 판단에는 사용하지 않음 (자세한 내용은 `docs/AUTO_TRADE_STRATEGY.md`)
- 매도: 손절 → 트레일링 → 부분 익절 → 채널 이탈 → 추세 이탈 → 모멘텀 역전 순으로 평가
- 프로필(`AGGRESSIVE/BALANCED/CONSERVATIVE`)에 따라 신호 민감도 자동 조정
- 설정된 **모든** 마켓을 순회하며 판단 (마켓별 일시정지 가능)
...
- OAuth2 로그인 사용 시 아래 설정 필요
  ...
  - `APP_AUTH_SUCCESS_REDIRECT_URL` (예: `/`)
  - `APP_AUTH_FAILURE_REDIRECT_URL` (예: `/?loginError=true`)
